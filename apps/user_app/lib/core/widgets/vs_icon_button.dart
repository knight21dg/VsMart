...
class VSIconButton extends StatelessWidget {
  ...
  /// Accessible name announced by screen readers for this icon-only button.
  /// Falls back to [tooltip] when omitted. Since the button renders only an
  /// icon, without this (or a tooltip) it has no accessible name.
  ...
  Widget build(BuildContext context) {
    ...
    // Give the icon-only button an accessible name and button role so screen
    // readers announce something meaningful (not just "button").
    final label = semanticLabel ?? tooltip;
    final labelled = label == null
        ? content
        : Semantics(button: true, label: label, child: content);
