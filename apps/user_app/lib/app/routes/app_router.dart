...
import '../../features/collections/presentation/collection_confirm_screen.dart';
import '../../features/credit/presentation/screens/cibil_score_screen.dart';
import '../../features/credit/presentation/screens/credit_application_status_screen.dart';
import '../../features/home/presentation/screens/home_screen.dart';
import '../../features/kyc/presentation/screens/credit_kyc_screen.dart';
...
final goRouterProvider = Provider<GoRouter>((ref) {
  ...
  ref
    ...
    // A parked deep link is not listenable state, so this is what re-runs the
    // redirect when one arrives while the app is already open and settled.
    ..listen(deepLinkTickProvider, (_, __) => refresh.value++);

  /// Every gate, in order. Returns a redirect target, or null when [loc] is
  /// allowed to render as-is.
  String? gateRedirect(String loc) {
    ...
    // Authenticated: the verification lifecycle guard, then the serviceability
    // HARD LOCK (GPS-based verdict) — composed in that order. The serviceability
    // lock only engages once registration is complete; running it for a
    // still-registering user loops /register <-> /not-serviceable and lands them
    // on the "Route not found" error page. See resolveAuthenticatedRedirect.
    return resolveAuthenticatedRedirect(
        stage, ref.read(serviceabilityGateProvider), loc);
  }
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      final gated = gateRedirect(loc);
      if (gated != null) return gated;

      // Every gate allowed this location, so the app is finally in a state where
      // a link can be shown. Open one if it's been waiting — this is what makes a
      // shared link survive the splash hold, onboarding and sign-in.
      final pending = ref.read(pendingDeepLinkProvider).take();
      return (pending == null || pending == loc) ? null : pending;
    },
    routes: [
      GoRoute(
      ...
      // ----- Credit (full-screen) -----
      GoRoute(
        path: RoutePaths.cibilScore,
        name: RouteNames.cibilScore,
        builder: (_, __) => const CibilScoreScreen(),
      ),
      GoRoute(
        path: RoutePaths.creditApplicationStatus,
        name: RouteNames.creditApplicationStatus,
        builder: (_, __) => const CreditApplicationStatusScreen(),
      ),
      GoRoute(
        path: RoutePaths.collectionConfirm,
        name: RouteNames.collectionConfirm,
        builder: (_, __) => const CollectionConfirmScreen(),
      ),
      ...
      GoRoute(
        path: RoutePaths.creditKyc,
        name: RouteNames.creditKyc,
        builder: (_, __) => const CreditKycScreen(),
      ),
      GoRoute(
