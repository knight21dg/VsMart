...
class AddressController extends Notifier<List<Address>> {
  ...
  Future<void> _hydrate() async {
    try {
      ...
      state = items;
    } catch (_) {/* controller disposed mid-flight — ignore */} finally {
      // Signal that the first backend hydration has settled so the UI can tell
      // a cold empty cache apart from a genuinely empty address book.
      try {
        ref.read(addressesHydratingProvider.notifier).state = false;
...
/// True while the first backend hydration of [addressesProvider] is in flight.
/// Lets the address list distinguish "still loading" from "no addresses yet".
final addressesHydratingProvider = StateProvider<bool>((ref) => true);
