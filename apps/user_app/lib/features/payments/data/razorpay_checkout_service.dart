...
class RazorpayCheckoutService {
  Future<PaymentOutcome> open(
    ...
    razorpay.on(Razorpay.EVENT_PAYMENT_SUCCESS, (PaymentSuccessResponse r) {
      // Keep the order id and signature: the backend re-computes the HMAC over
      // `order_id|payment_id` to verify this callback. Only `paymentId` used to
      // be kept, so the client's "success" was never provable server-side.
