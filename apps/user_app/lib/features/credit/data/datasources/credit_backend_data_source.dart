...
class CreditBackendDataSource implements CreditDataSource {
  ...
  @override
  Future<CibilReport?> getLatestCibil() async {
    final res = await _client.get<dynamic>(ApiConstants.creditCibil);
    // Envelope: { data: { report: {...} | null } }
    final report = _obj(res.data)['report'];
    return report is Map
        ? CibilReport.fromJson(Map<String, dynamic>.from(report))
        : null;
  }

  @override
  Future<CibilReport> checkCibil({
    required String mobile,
    required bool consent,
  }) async {
    final res = await _client.post<dynamic>(
      ApiConstants.creditCibilCheck,
      data: {'mobile': mobile, 'consent': consent},
    );
    final body = res.data;
    // A "no record" answer comes back HTTP 200 with code CIBIL_NO_RECORD and no
    // data — surface it as a no-record report rather than throwing.
    final code = body is Map ? body['code']?.toString() : null;
    if (code == 'CIBIL_NO_RECORD') {
      return CibilReport(
        status: CibilStatus.noRecord,
        score: 0,
        band: '',
        nameOnBureau: '',
        pan: '',
    ...
    // Success: CIBIL_FETCHED — the report fields sit directly under `data`.
    return CibilReport.fromJson(_obj(body));
  }
