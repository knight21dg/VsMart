...
/// The customer's most recent stored CIBIL report (null if never checked).
final cibilLatestProvider = FutureProvider<CibilReport?>(
  (ref) async => _unwrap(await ref.watch(creditRepositoryProvider).getLatestCibil()),
);

/// Drives a self-service CIBIL check. `state` mirrors the in-flight / result /
/// failure so the screen can render a spinner and route failures through the
/// actionable presenter. On success it refreshes [cibilLatestProvider].
class CibilCheckController extends AsyncNotifier<CibilReport?> {
  @override
  Future<CibilReport?> build() async => null;

  Future<CibilReport?> check({
    ...
    final result = await ref
        ...
        .checkCibil(mobile: mobile, consent: consent);
    return result.fold(
      (failure) {
...
final cibilCheckControllerProvider =
    AsyncNotifierProvider<CibilCheckController, CibilReport?>(
  CibilCheckController.new,
);
