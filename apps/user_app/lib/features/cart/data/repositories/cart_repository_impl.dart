...
class CartRepositoryImpl implements CartRepository {
  ...
  Future<Cart> addItem(CartItem item) async {
    final items = _read();
    // Match the LINE, not the product: two variants of one product are two
    // lines. (For a variant-less product lineKey == productId, so this is
  ...
  @override
  Future<Cart> setQuantity(String lineKey, int quantity) async {
    final items = _read();
    final index = items.indexWhere((i) => i.lineKey == lineKey);
  ...
  @override
  Future<Cart> removeItem(String lineKey) async {
    final items = _read()..removeWhere((i) => i.lineKey == lineKey);
    return _write(items);
  }
