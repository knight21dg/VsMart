...
class _Stepper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final vs = context.vsColors;
    Widget btn(IconData icon, VoidCallback onTap, String label) => Semantics(
          button: true,
          label: label,
          child: InkWell(
            onTap: onTap,
      ...
      child: Row(
        ...
        children: [
          btn(
              ...
              onDecrement,
              quantity > 1 ? 'Decrease quantity' : 'Remove item'),
          Semantics(
            label: 'Quantity',
            value: '$quantity',
            child: Text('$quantity',
          ...
          btn(Icons.add_rounded, onIncrement, 'Increase quantity'),
        ],
      ),
    );
  }
...
class VSCartSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          _row(context, context.l10n.checkoutItemTotal, summary.subtotal.asCurrency),
          if (summary.savings > 0) ...[
            AppSpacing.vGapSm,
            _row(context, context.l10n.checkoutSavings, '- ${summary.savings.asCurrency}',
                color: vs.success),
          ],
          AppSpacing.vGapSm,
          ...
          if (summary.gstAmount > 0) ...[
            AppSpacing.vGapSm,
            _row(context, context.l10n.cartGst, summary.gstAmount.asCurrency),
          ],
...
class VSCartFooter extends StatelessWidget {
  ...
  /// Trailing icon on the CTA — an arrow for checkout, or e.g. a location pin
  /// when the button is prompting the shopper to add a delivery address.
  final IconData icon;
...
class VSEmptyCart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return VSEmptyState(
      ...
      actionLabel: context.l10n.commonStartShopping,
