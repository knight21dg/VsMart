...
extension OrderStatusX on OrderStatus {
  ...
  /// Whether the customer can still cancel from this state. Mirrors the backend
  /// `cancel_order` guard (orders/services.py allows pending|confirmed only), so
  /// the app never offers a Cancel action the server will reject with
  ...
  /// The goods are physically with a rider right now.
  ///
  /// This is the only state where a live map and a "call your partner" action
  /// mean anything — there is a real person with a real position to show.
  bool get isDispatched => this == OrderStatus.outForDelivery;

  /// Whether LIVE TRACKING should be offered at all.
  ///
  /// [isActive] was being used as the proxy for this everywhere, but it is true
  /// for `draft`, `pending`, `placed` and `failedDelivery` — so an order placed
  /// thirty seconds ago opened a full-screen map with a simulated rider driving
  /// toward the customer, and a cancelled delivery kept one open. Tracking starts
  /// once the store has actually handed the parcel over.
  bool get isTrackable =>
      this == OrderStatus.readyForDispatch || this == OrderStatus.outForDelivery;

  /// A terminal state that did NOT result in a delivery — the customer needs an
  /// explanation, not a map.
  bool get isFailed =>
      isCancelled || this == OrderStatus.failedDelivery;

  /// Whether the order is being prepared but hasn't left the store yet. These
  /// show progress (packed / ready to ship) without any live-location theatre.
