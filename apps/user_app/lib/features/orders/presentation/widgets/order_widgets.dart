...
class VSOrderStatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return VSStatusChip(
      label: status.labelL10n(context.l10n),
...
class _Row extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Localize timeline node labels at render (the datasource stores context-free
    // English fallbacks). The first "pending" node is the order-created milestone.
    final label = entry.status == OrderStatus.pending
        ? context.l10n.ordersOrderPlaced
        : entry.status.labelL10n(context.l10n);
...
class VSOrderSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        row(context.l10n.orderItemTotal, summary.itemTotal.asCurrency),
        row(
        ...
        row(context.l10n.orderGrandTotal, summary.grandTotal.asCurrency, bold: true),
      ],
    );
  }
}
