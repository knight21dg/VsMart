...
class ProductSectionScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        error: (e, _) => VSErrorView(
          ...
          onRetry: () => _invalidateProvider(ref),
        ),
        data: (products) {
          if (products.isEmpty) {
            return VSEmptyState(
              icon: _emptyIcon,
              title: context.l10n.catalogNothingHere,
  ...
  /// Re-fetch the section's backing provider after a failed load.
  void _invalidateProvider(WidgetRef ref) {
    switch (section) {
      case ProductSection.popular:
      ...
      case ProductSection.recentlyOrdered:
        ref.invalidate(recentlyOrderedProductsProvider);
      case ProductSection.sales:
        ref.invalidate(dealsProductsProvider);
    }
  }

  /// Pull-to-refresh: invalidate and await the section's backing provider.
  Future<void> _refresh(WidgetRef ref) async {
    _invalidateProvider(ref);
    switch (section) {
      case ProductSection.popular:
        await ref.read(popularProductsProvider.future);
      case ProductSection.recommended:
        await ref.read(recommendedProductsProvider.future);
      case ProductSection.recentlyOrdered:
        await ref.read(recentlyOrderedProductsProvider.future);
      case ProductSection.sales:
        await ref.read(dealsProductsProvider.future);
    }
  }
}
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(context.l10n.cartItemsCount(count),
                    style:
...
class _ProductGridItem extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return VSProductCard(
      ...
      outOfStock: !product.effectiveInStock,
      ...
      onShareTap: () => shareProductLink(context, ref, product),
      onTap: () =>
          openProductDetail(context, productId: product.id, source: 'section'),
      onAdd: () => addToCartOrChoose(context, ref, product, source: 'section'),
      onIncrement: () =>
