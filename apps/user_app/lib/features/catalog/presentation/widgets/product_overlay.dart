...
import 'product_suggestions_rail.dart';
...
class _ProductOverlayState extends State<_ProductOverlay> {
  ...
  /// Open a product tapped in the suggestions rail. If it's already one of this
  /// overlay's pages we just swipe to it (staying in the overlay); otherwise the
  /// overlay closes and hands off to the full detail route — never stack a second
  /// product surface on top of this one.
  void _openSuggested(Product product) {
    final i = widget.products.indexWhere((x) => x.id == product.id);
    if (i >= 0) {
      _onDockChanged(i);
      return;
    }
    Navigator.of(context).pop();
    openProductDetail(context, productId: product.id, source: 'suggestions');
  }
            ...
            children: [
              ...
              _RoundIconButton(
                ...
                onTap: () {
                  ...
                  context.showSnack(context.l10n.catalogShareLinkCopied);
                },
              ),
            ],
          ),
...
class _ProductPage extends ConsumerStatefulWidget {
  ...
  /// Open another product from the suggestions rail — the overlay decides
  /// whether to swipe to it (already one of its pages) or hand off to the full
  /// detail route.
        ...
        children: [
          ...
          widget.active
              ? Hero(
                  ...
                  // Fly the SAME image the source card had cached (its
                  // thumbnail) so the flight never shimmers.
                  flightShuttleBuilder: (_, __, ___, ____, _____) =>
                      VSNetworkImage(
                    url: p.imageUrl ??
                        (p.gallery.isNotEmpty ? p.gallery.first : null),
                    fit: BoxFit.cover,
                  ),
                  child: VSProductGallery(
                      images: p.gallery, heroFallbackUrl: p.imageUrl),
                )
              : VSProductGallery(
                  images: p.gallery, heroFallbackUrl: p.imageUrl),
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // No rating/reviews on products (groceries) — feedback is
                // captured per-ORDER instead. Matches the full detail page.
                AppSpacing.vGapMd,
          ...
          // Suggestions — this overlay IS a product detail surface, so it gets
          // the same rail as the full page. Full-bleed (outside the padding) so
          // the rail scrolls edge-to-edge like every other rail.
          const Divider(height: AppSpacing.xxl),
          ProductSuggestionsRail(
            excludeId: p.id,
            onTapProduct: widget.onOpenProduct,
          ),
...
class _SharedCta extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Key on the LINE the Add button creates (productId, or productId:variantId
    // once a variant is picked) — stepping the bare productId would drive the
    // wrong line for a variant.
    final lineKey = detail.selectedVariant == null
        ? product.id
        : '${product.id}:${detail.selectedVariant!.id}';
    final qty = ref.watch(
        cartControllerProvider.select((c) => c.quantityOf(lineKey)));
                ...
                ? Row(
                    children: [
                      _MiniStepper(
                        ...
                        onAdd: () {
                          HapticFeedback.selectionClick();
                          cart.increment(lineKey);
                        },
                        onRemove: () {
                          HapticFeedback.selectionClick();
                          cart.decrement(lineKey);
                        },
                      ),
                      AppSpacing.hGapSm,
                      Expanded(
                ...
                : VSButton(
                    ...
                    onPressed: enabled
                        ? () {
                            ...
                            // Same as the detail screen: carry the selected
                            // variant so the charged price matches the shown one.
                            ...
                            context.showSnack(
                                context.l10n.catalogAddedToCart(product.name));
                          }
                        : null,
                  ),
          ),
