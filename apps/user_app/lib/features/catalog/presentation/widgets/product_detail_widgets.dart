    ...
    return Wrap(
      ...
      children: [
        for (var i = 0; i < variants.length; i++)
          _VariantChip(
            // Each pack is stocked on its own, so a sold-out 1 kg strikes through
            // even while the 500 g is available.
            label: variants[i].isSellable
                ? variants[i].label
                : '${variants[i].label} · sold out',
            selected: i == selectedIndex,
            enabled: variants[i].isSellable,
            onTap: variants[i].isSellable ? () => onSelect(i) : null,
      ...
      child: Row(
        ...
        children: [
          IconButton(
            ...
            tooltip: context.l10n.catalogDecreaseQuantity,
            icon: const Icon(Icons.remove_rounded, size: 18),
            onPressed: quantity > 1 ? () => onChanged(quantity - 1) : null,
          ),
          Semantics(
            label: context.l10n.catalogQuantity,
            value: '$quantity',
            child: Text('$quantity', style: AppTypography.titleMedium),
          ),
          IconButton(
            visualDensity: VisualDensity.compact,
            tooltip: context.l10n.catalogIncreaseQuantity,
...
class VSStockStatus extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return switch (status) {
      StockStatus.inStock => VSStatusChip(
          label: context.l10n.catalogInStock,
      ...
      StockStatus.outOfStock => VSStatusChip(
          label: context.l10n.productOutOfStock,
...
class VSSpecificationSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(context.l10n.catalogSpecifications,
