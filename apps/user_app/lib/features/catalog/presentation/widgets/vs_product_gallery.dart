...
class VSProductGallery extends StatefulWidget {
  const VSProductGallery({super.key, required this.images, this.heroFallbackUrl});

  final List<String> images;

  /// The image the opening card already had cached (its thumbnail). Used as the
  /// warm placeholder for the FIRST gallery image so a first open shows the
  /// picture instantly and cross-fades — never a shimmer on the hero.
  final String? heroFallbackUrl;
          ...
          child: PageView.builder(
            ...
            itemBuilder: (_, i) => GestureDetector(
              ...
              child: VSNetworkImage(
                url: widget.images[i],
                fit: BoxFit.cover,
                // Only the first slot morphs from the tapped card, so give it the
                // warm thumbnail as its placeholder — no shimmer on first open.
                placeholderUrl: i == 0 ? widget.heroFallbackUrl : null,
              ),
            ),
          ),
        ),
