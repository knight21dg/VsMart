...
class ProductFilter extends Equatable {
  ...
  ///
  /// Takes [AppLocalizations] so the chip labels are localized at the call site
  /// (this entity stays free of BuildContext).
  List<({String label, ProductFilter cleared})> activeChips(AppLocalizations l) {
    final chips = <({String label, ProductFilter cleared})>[];
    if (inStockOnly) {
      chips.add(
          (label: l.catalogInStock, cleared: copyWith(inStockOnly: false)));
    }
    if (minPrice != null || maxPrice != null) {
      final lo = minPrice?.round();
      final hi = maxPrice?.round();
      chips.add((
        label: l.catalogPriceRange('${lo ?? 0}', hi?.toString() ?? '∞'),
    ...
    if (minimumDiscount != null) {
      chips.add((
        label: l.catalogDiscountOff('${minimumDiscount!.round()}'),
