...
import '../../../../core/services/notification_route.dart';
...
class _NotificationsScreenState extends ConsumerState<NotificationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasUnread =
        (async.valueOrNull ?? const []).any((n) => !n.read);
          ...
          data: (all) {
            ...
            if (items.isEmpty) {
              return VSEmptyState(
                ...
                message: context.l10n.notificationsAllCaughtUp,
                icon: Icons.notifications_none_rounded,
              );
            }
            final today = items.where((n) => n.time.isToday).toList();
            final yesterday =
                items.where((n) => n.time.isYesterday).toList();
            final earlier = items
            ...
            return RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  if (today.isNotEmpty)
                    _Group(
                        label: context.l10n.notificationsToday,
                        items: today,
                        onOpen: _open),
                  if (yesterday.isNotEmpty)
                    _Group(
                        label: context.l10n.notificationsYesterday,
                        items: yesterday,
                        onOpen: _open),
                  if (earlier.isNotEmpty)
                    _Group(
                        label: context.l10n.notificationsEarlier,
                        items: earlier,
                        onOpen: _open),
                ],
              ),
  ...
  void _open(AppNotification n) {
    ...
    // `route` is a PATH, not a route name — that's the contract push already
    // uses, and only a path can carry parameters (`/orders/VS12345`). This used
    // to call pushNamed() on the same string, so every order / invoice /
    // statement / ticket notification threw when tapped in-app.
    final path = resolveNotificationPath(n.route);
    if (path == null) return;
    context.push(path);
  }
}
