...
class NotificationPreferencesController
  ...
  Future<void> setChannel(String key, bool value) async {
    ...
    try {
      ...
      // Roll the optimistic toggle back, and surface the failure so the screen
      // can present it (providers must not hold a BuildContext).
      ref.read(lastNotificationPrefsFailureProvider.notifier).state =
...
/// Holds the [Failure] from the most recent failed preference PATCH so the
/// settings screen can route it through the actionable error presenter. The
/// optimistic toggle is still rolled back; this only adds user feedback. The
/// screen clears it after presenting.
final lastNotificationPrefsFailureProvider =
    StateProvider<Failure?>((ref) => null);
