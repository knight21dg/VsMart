...
class FamilyInfoScreen extends ConsumerWidget {
  ...
  List<_Member> _members(
      BuildContext context, WidgetRef ref, FamilyGroupModel group) {
    ...
    return [
      _Member(
        ...
        relationship: context.l10n.profilePrimaryHolder,
        phone: user?.phone ?? '',
      ...
      for (final m in group.members)
        _Member(
          id: m.id,
          name: m.relationship.isEmpty
              ? context.l10n.profileFamilyMember
              : m.relationship,
          relationship: m.status == 'pending'
              ? context.l10n.profileInvitationPending
              : context.l10n.profileHouseholdMember,
          phone: m.phone,
  ...
  Future<void> _removeMember(
    ...
    try {
      ...
      if (context.mounted) {
        context.showSnack(context.l10n.profileMemberRemoved(member.name));
      }
    } catch (e) {
  ...
  Future<void> _addMember(BuildContext context, WidgetRef ref) async {
    ...
    final added = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(context.l10n.profileInviteMember),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
              ...
              decoration:
                  InputDecoration(labelText: context.l10n.addressPhone),
            ),
            TextField(
              controller: relationController,
              decoration: InputDecoration(
                  labelText: context.l10n.profileRelationshipHint),
            ),
          ],
        ),
        actions: [
          ...
          FilledButton(
            ...
            child: Text(context.l10n.profileInvite),
          ),
        ],
      ),
    );
    ...
    // Validate the invite number properly (10 digits) before hitting the API.
    final phoneError = Validators.phone(phone);
    if (phoneError != null) {
      if (context.mounted) context.showSnack(phoneError, isError: true);
      return;
    }
    try {
      ...
      ref.invalidate(familyGroupProvider);
      if (context.mounted) {
        context.showSnack(context.l10n.profileInviteSent(phone));
      }
    } catch (e) {
      if (!context.mounted) return;
      presentFailure(context, ref, ErrorHandler.handle(e));
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: VSAppBar(title: context.l10n.profileFamilyInfo),
      body: SafeArea(
        child: groupAsync.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          // Pass the coded failure through so an auth error routes to login
          // instead of dead-ending on a generic message with a Retry that can
          // never succeed (the KYC screen had exactly this bug).
          error: (e, __) => VSErrorView(
            failure: e is Failure ? e : null,
            message: e is Failure ? null : context.l10n.profileHouseholdLoadError,
            onRetry: () => ref.invalidate(familyGroupProvider),
          ),
          data: (group) {
            final members = _members(context, ref, group);
            return SingleChildScrollView(
              padding: AppSpacing.screen,
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(context.l10n.profileHouseholdMembers,
                      style: AppTypography.headlineLarge),
                  AppSpacing.vGapSm,
                  Text(
                    context.l10n.profileFamilySubtitle,
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          context.l10n.profileSharedLimitUsage,
                          style: AppTypography.bodySmall
                              .copyWith(color: vs.textSecondary),
                        ),
                        Text(
...
class _TrailingAffordance extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return switch (member.status) {
      _MemberStatus.active => VSStatusChip(
          label: context.l10n.profileActive,
          tone: VSStatusTone.success,
          icon: Icons.verified_user_outlined,
          dense: true,
        ),
      _MemberStatus.pending => VSStatusChip(
          label: context.l10n.orderStatusPending,
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          AppSpacing.vGapMd,
          Text(
            context.l10n.profileAddHouseholdMember,
            style: AppTypography.titleLarge,
            textAlign: TextAlign.center,
          ),
          AppSpacing.vGapXs,
          Text(
            context.l10n.profileAddMemberBody,
            textAlign: TextAlign.center,
            style: AppTypography.bodyMedium.copyWith(color: vs.textSecondary),
          ),
          AppSpacing.vGapLg,
          VSButton(
            label: context.l10n.profileAddMember,
