...
import '../../../orders/presentation/widgets/delivery_otp_card.dart';
...
class _AccountScreenState extends ConsumerState<AccountScreen> {
  ...
  Future<void> _confirmLogout() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(context.l10n.profileLogoutConfirmBody),
        actions: [
          TextButton(
  ...
  Future<void> _refresh() async {
    ...
    if (ref.read(creditAccessProvider).isActive) {
      ...
      body: SafeArea(
        ...
        child: RefreshIndicator(
          ...
          children: [
            ...
            AppSpacing.vGapMd,
            // Order at the door: surface the handover OTP HERE, where the
            // customer already is — it used to live only in the notification
            // inbox while the rider stood waiting for it.
            const _ArrivingOrderBanner(),
            if (access.isActive)
            ...
            if (user != null) ...[
              OutlinedButton.icon(
                ...
                label: Text(context.l10n.accountLogout),
              ),
              AppSpacing.vGapMd,
            ],
            Center(
              child: Text(
                '${AppConstants.appName} · ${context.l10n.accountVersion('1.0.0')}',
        ...
        children: [
          Row(
            children: [
              ...
              AppSpacing.hGapSm,
              Text(context.l10n.profileBrowsingAsGuest,
          ...
          AppSpacing.vGapSm,
          Text(
            context.l10n.profileGuestSignInBody,
          ...
          SizedBox(
            ...
            child: FilledButton(
              ...
              child: Text(context.l10n.profileSignInCreate,
...
class _ProfileTopCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final name = (user?.name).isNullOrBlank ? context.l10n.profileGuest : user!.name;
        ...
        children: [
          ...
          if (showCredit) ...[
            ...
            Column(
              ...
              children: [
                Text(
                    context.l10n.profileCreditAmount(
                        (account?.available ?? 0).asCurrencyCompact),
                    style: AppTypography.labelMedium),
                Text(
                    context.l10n
                        .profileScoreValue('${account?.vsScore ?? '—'}'),
                    style: AppTypography.bodySmall
                        .copyWith(color: vs.textSecondary)),
              ],
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(context.l10n.profileUsedAmount(used.asCurrency),
                  style: AppTypography.bodySmall.copyWith(color: faint)),
              Text(context.l10n.profileLimitAmount(limit.asCurrency),
                  style: AppTypography.bodySmall.copyWith(color: faint)),
            ],
          ),
          AppSpacing.vGapMd,
            ...
            children: [
              Expanded(
                child: FilledButton(
                  ...
                  child: Text(context.l10n.billingViewStatement,
                      style: AppTypography.labelMedium
                          .copyWith(color: AppColors.white)),
                ),
              ),
              ...
              Expanded(
                child: FilledButton(
                  ...
                  child: Text(context.l10n.profilePayDue,
                      style: AppTypography.labelMedium
                          .copyWith(color: AppColors.trustBlue)),
                ),
              ),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  AppSpacing.hGapSm,
                  Text(
                    '${order.status.labelL10n(context.l10n)} · ${order.summary.grandTotal.asCurrency}',
                    style: AppTypography.bodySmall
                        .copyWith(color: vs.textSecondary),
                  ),
                ],
...
class _QuickGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = <_QuickItem>[
      _QuickItem(Icons.receipt_long_outlined, context.l10n.navOrders, vs.brand,
          () => context.pushNamed(RouteNames.orders)),
      _QuickItem(Icons.location_on_outlined, context.l10n.profileAddresses,
          vs.trust, () => context.pushNamed(RouteNames.addresses)),
      _QuickItem(Icons.account_balance_wallet_outlined, context.l10n.navCredit,
          vs.offer, () => context.goNamed(RouteNames.creditDashboard)),
      _QuickItem(Icons.favorite_border_rounded, context.l10n.wishlistTitle,
          AppColors.error, () => context.pushNamed(RouteNames.wishlist)),
      _QuickItem(Icons.payments_outlined, context.l10n.profilePayments, vs.brand,
          () => context.pushNamed(RouteNames.paymentHistory)),
      _QuickItem(Icons.local_offer_outlined, context.l10n.notifOffers, vs.offer,
          () => context.pushNamed(RouteNames.offers)),
      _QuickItem(Icons.description_outlined, context.l10n.creditStatements,
          vs.trust, () => context.pushNamed(RouteNames.statements)),
      _QuickItem(Icons.headset_mic_outlined, context.l10n.profileSupport,
          vs.brand, () => context.pushNamed(RouteNames.support)),
    ];
    return _Card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(context.l10n.profileQuickAccess,
              style: AppTypography.titleLarge),
          AppSpacing.vGapMd,
          GridView.count(
            crossAxisCount: 4,
...
class _CreditCenterCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rows = <(IconData, String, Color, String)>[
      (Icons.description_outlined, context.l10n.profileMonthlyStatement,
          vs.brand, RouteNames.statements),
      (Icons.account_balance_wallet_outlined, context.l10n.profileOutstandingDue,
          AppColors.error, RouteNames.outstandingDue),
      (Icons.bar_chart_rounded, context.l10n.profileCreditUsage, vs.trust,
          RouteNames.paymentHistory),
      (Icons.speed_rounded, context.l10n.profileVsScoreDetails, vs.offer,
    ...
    return _Card(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(context.l10n.profileCreditCenter,
              style: AppTypography.titleLarge),
          AppSpacing.vGapSm,
        ...
        children: [
          Row(
            children: [
              ...
              AppSpacing.hGapSm,
              Text(context.l10n.addressHome, style: AppTypography.titleMedium),
              const Spacer(),
              GestureDetector(
          ...
          AppSpacing.vGapSm,
          Text(
            address?.formatted ?? context.l10n.profileNoSavedAddress,
          ...
          SizedBox(
            ...
            child: FilledButton(
              ...
              child: Text(context.l10n.profileManageAddresses,
...
class _RecentPaymentsCard extends ConsumerWidget {
  ...
  ({IconData icon, String label}) _method(AppLocalizations l, RepaymentMethod m) =>
      switch (m) {
        RepaymentMethod.upi =>
          (icon: Icons.account_balance_rounded, label: l.profilePaymentUpi),
        RepaymentMethod.card =>
          (icon: Icons.credit_card_rounded, label: l.profilePaymentCard),
        RepaymentMethod.bankTransfer => (
            icon: Icons.account_balance_outlined,
            label: l.profilePaymentBankTransfer
          ),
        RepaymentMethod.cashCollection =>
          (icon: Icons.local_atm_rounded, label: l.profilePaymentCashCollection),
      };
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _Card(
      child: Column(
        ...
        children: [
          _CardHeader(
            title: context.l10n.profileRecentPayments,
            actionLabel: context.l10n.profileViewHistory,
          ...
          paymentsAsync.when(
            ...
            error: (_, __) => Padding(
              padding: const EdgeInsets.symmetric(vertical: AppSpacing.md),
              child: Text(context.l10n.profileCouldNotLoadPayments,
                  style: AppTypography.bodySmall
                      .copyWith(color: vs.textSecondary)),
            ),
            data: (payments) {
              if (payments.isEmpty) {
                return Padding(
                  padding: const EdgeInsets.symmetric(vertical: AppSpacing.md),
                  child: Text(context.l10n.billingNoPayments,
                      style: AppTypography.bodySmall
                          .copyWith(color: vs.textSecondary)),
                );
              }
              ...
              return Column(
                children: [
                  for (final Repayment p in recent)
                    _PaymentRow(
                      data: _method(context.l10n, p.method),
...
class _KycStatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      context.l10n.profileKycAadhaar,
      context.l10n.kycDocPan,
      context.l10n.profileKycSelfie,
      context.l10n.profileKycHouse,
    ];
    return _Card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(context.l10n.profileKycStatus,
              style: AppTypography.titleLarge),
          AppSpacing.vGapMd,
          GridView.count(
            crossAxisCount: 2,
...
/// Live reward-points balance. The Rewards screen was previously reachable only
/// from a row buried in Support & Settings, so the balance itself was never
/// visible anywhere in the app — this puts the number on the profile hub.
class _LoyaltyPointsCard extends ConsumerWidget {
  const _LoyaltyPointsCard();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Loyalty is a nice-to-have on this screen: while it loads, and if it
    // fails outright, the card simply stays out of the way rather than
    // pushing an error into the middle of the profile.
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.stars_rounded, color: vs.offer),
              ),
              AppSpacing.hGapMd,
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(context.l10n.loyaltyRewardPoints,
                        style: AppTypography.titleMedium),
                    Text(
                      '${Formatters.number(s.balance)} '
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(context.l10n.profileOffersRewards,
                    style: AppTypography.titleMedium),
                Text(context.l10n.profileActiveCoupons(3),
                    style: AppTypography.bodySmall
                        .copyWith(color: vs.textSecondary)),
              ],
            ),
          ...
          OutlinedButton(
            ...
            child: Text(context.l10n.profileViewOffers,
                style: AppTypography.labelSmall.copyWith(color: vs.brand)),
          ),
        ],
      ),
      ...
      child: Column(
        children: [
          _ListRow(
            ...
            label: context.l10n.profileFaqHelp,
            color: vs.trust,
          ...
          _ListRow(
            ...
            label: context.l10n.profileMyReturns,
            color: vs.trust,
          ...
          _ListRow(
            ...
            label: context.l10n.profileReferEarn,
            color: vs.offer,
          ...
          _ListRow(
            ...
            label: context.l10n.profileLanguageEnglish,
            color: vs.offer,
          ...
          _ListRow(
            ...
            label: context.l10n.profileAboutVsMart,
          ...
          _ListRow(
            ...
            label: context.l10n.profileCareers,
            color: vs.textSecondary,
...
/// Shown only while an order is genuinely OUT WITH A RIDER: one tap to the live
/// tracking, with the delivery OTP inline when it's still spendable.
class _ArrivingOrderBanner extends ConsumerWidget {
  const _ArrivingOrderBanner();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final orders = ref.watch(ordersProvider).valueOrNull ?? const <Order>[];
    final arriving =
        orders.where((o) => o.status.isDispatched).toList(growable: false);
    if (arriving.isEmpty) return const SizedBox.shrink();
    final order = arriving.first;
    final tracking =
        ref.watch(orderTrackingProvider(order.id)).valueOrNull;
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.local_shipping_rounded, color: vs.brand),
                  AppSpacing.hGapSm,
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(context.l10n.profileOrderArriving,
                            style: AppTypography.titleMedium),
                        Text(
                          context.l10n.profileShowOtp,
                          style: AppTypography.bodySmall
                              .copyWith(color: vs.textSecondary),
                        ),
                      ],
              ...
              if (tracking != null && tracking.hasDeliveryOtp) ...[
                AppSpacing.vGapSm,
                DeliveryOtpCard(code: tracking.deliveryOtp, compact: true),
              ],
            ],
          ),
        ),
