...
class ServiceabilityController extends AsyncNotifier<ServiceabilityResult> {
  ...
  Future<ServiceabilityResult> build() async {
    ...
    // Resolution priority:
    //   1. a location the customer PICKED via change-location (manual override) —
    //      so a banner/lock-screen pin-drop re-resolves the verdict for the NEW
    //      spot instead of reverting to the old address on the next rebuild;
    //   2. the selected delivery address (the normal address-book flow);
    //   3. the live device (NEARBY) location, so serviceability + the serving
    //      store resolve even before the customer has saved an address.
    final manual = ref.watch(manualLocationProvider);
    final address = ref.watch(selectedAddressProvider);
    final live = ref.watch(resolvedLocationProvider);
    final lat = manual?.latitude ?? address?.latitude ?? live?.latitude;
    final lng = manual?.longitude ?? address?.longitude ?? live?.longitude;
    final pincode = manual?.pincode ?? address?.pincode ?? live?.pincode;
    if (lat == null && lng == null && (pincode == null || pincode.isEmpty)) {
...
/// The last serviceability verdict persisted to disk (survives app restarts), or
/// null when none has been stored yet. The serviceability gate hydrates from this
/// so a returning, previously-serviceable customer opens straight to Home without
/// waiting on a fresh GPS check — the fresh check runs in the background on Home.
ServiceabilityResult? readCachedServiceability(Ref ref) {
  try {
    final raw = ref.read(hiveServiceProvider).cacheBox.get(_cacheKey);
    if (raw is String && raw.isNotEmpty) {
      return ServiceabilityResult.fromJson(
