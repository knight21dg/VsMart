...
class ServiceabilityGateController extends Notifier<GateStatus> {
  ...
  @override
  GateStatus build() {
    // Open fast for returning users: if the LAST persisted verdict was positively
    // serviceable, start there so the app goes straight to Home — no GPS wait on
    // this launch. Home then re-checks in the background and the home banner shows
    // "we're not here" if the customer has since moved out of the area.
    //
    // A cached 'not serviceable' is deliberately NOT trusted here (it's ambiguous
    // with the "unknown" sentinel), so those users get a fresh check on open.
    final cached = readCachedServiceability(ref);
    if (cached != null && cached.serviceable) {
      _started = true; // treat the cached verdict as this session's baseline
      return GateStatus.serviceable;
    }
    return GateStatus.unresolved;
  }
