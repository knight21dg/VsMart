...
///
/// Shows the customer's real total outstanding amount with its due date, a
/// guidance banner, a credit-utilization summary, and a sticky "Pay Now" CTA
/// that routes to the repayment flow. All figures are backend-sourced; the
/// screen never invents a principal / interest / late-fee split (VS Mart lends
/// through an NBFC/LSP partner and does not present its own loan breakdown).
class OutstandingDueScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: VSAppBar(title: context.l10n.billingOutstandingDue),
      body: accountAsync.when(
        loading: () => const VSLoadingView(),
        // Coded failures route themselves (auth -> login); only fall back to
        // the generic copy for a plain exception.
        error: (e, __) => VSErrorView(
          failure: e is Failure ? e : null,
          message: e is Failure ? null : context.l10n.billingDuesLoadError,
          ...
          child: RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                  ...
                  AppSpacing.vGapLg,
                  Text(context.l10n.billingCreditSummary,
...
class _DueCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dueLabel = dueDate == null
        ? null
        : context.l10n.billingDueOnDate(
            DateFormat('d MMMM yyyy').format(dueDate!));
    final chipLabel = isOverdue
        ? context.l10n.billingOverdueByDays(-daysUntilDue)
        : context.l10n.billingDueInDays(daysUntilDue);
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
            context.l10n.billingTotalOutstandingAmount,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              context.l10n.billingPayBeforeDueNote,
        ...
        children: [
          ...
          Row(
            children: [
              ...
              AppSpacing.hGapSm,
              Text(
                context.l10n.billingUsedAmount(used.asCurrency),
                style: AppTypography.bodySmall.copyWith(
                  color: vs.textSecondary,
                ),
              ),
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    context.l10n.billingPayingTotalAmount,
                    style: AppTypography.bodySmall.copyWith(
                      color: vs.textSecondary,
                    ),
                  ),
