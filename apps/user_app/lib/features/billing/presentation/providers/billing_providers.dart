...
/// Authoritative limit + outstanding, read together from `/credit/dashboard`.
/// The dashboard's money figures derive from THIS, not from summing the ledger:
/// the ledger endpoint paginates (20 rows), so a client-side sum silently
/// undercounts any account with more history, showing a wrong outstanding.
final creditSummaryProvider = FutureProvider<({num limit, num outstanding})>(
  (ref) async =>
      _unwrap(await ref.watch(billingRepositoryProvider).getCreditSummary()),
);
...
final outstandingBalanceProvider = FutureProvider<num>((ref) async {
  final summary = await ref.watch(creditSummaryProvider.future);
  return summary.outstanding;
});

final availableCreditProvider = FutureProvider<num>((ref) async {
  final summary = await ref.watch(creditSummaryProvider.future);
  final available = summary.limit - summary.outstanding;
  return available < 0 ? 0 : available;
});

final creditUtilizationProvider = FutureProvider<double>((ref) async {
  final summary = await ref.watch(creditSummaryProvider.future);
  if (summary.limit <= 0) return 0;
  return (summary.outstanding / summary.limit).clamp(0, 1).toDouble();
});
...
final billingOverviewProvider = FutureProvider<BillingOverview>((ref) async {
  ...
  // Money figures from the backend authority; the ledger is only the recent-
  // activity list. Utilization is derived from the same authoritative pair, so
  // the hero and the Outstanding Due screen can no longer show two numbers.
  final summary = await ref.watch(creditSummaryProvider.future);
  final ledger = await ref.watch(creditLedgerProvider.future);
  final statement = await ref.watch(currentStatementProvider.future);
  final available = summary.limit - summary.outstanding;
  final utilization =
      summary.limit <= 0 ? 0.0 : (summary.outstanding / summary.limit).clamp(0, 1).toDouble();
  return (
    creditLimit: summary.limit,
...
/// it through the actionable error presenter (KYC_REQUIRED, credit gates…).
/// Cleared at the start of each pay.
final lastRepaymentFailureProvider = StateProvider<Failure?>((ref) => null);

void _refreshLedger(Ref ref) {
  ref
    // The authoritative outstanding lives here — without invalidating it the
    // dashboard hero keeps the pre-repayment balance even though the ledger
    // refreshed.
    ..invalidate(creditSummaryProvider)
...
class RepaymentController extends Notifier<bool> {
  ...
  Future<bool> pay({
    ...
    state = true;
    ref.read(lastRepaymentFailureProvider.notifier).state = null;
    try {
      ...
      // — e.g. KYC_REQUIRED / credit-gate envelopes drive the right UX instead
      // of a generic toast.
      ref.read(lastRepaymentFailureProvider.notifier).state =
