...
class BillingRepositoryImpl with BaseRepository implements BillingRepository {
  ...
  @override
  Future<Either<Failure, ({num limit, num outstanding})>> getCreditSummary() =>
      guard(dataSource.getCreditSummary, requireConnection: false);

  @override
  Future<Either<Failure, List<CreditLedgerEntry>>> getLedger() =>
      guard(() async {
        // Network-FIRST when online. This is money: the ledger changes server-side
        // on every repayment, purchase and fee, so the cache is an OFFLINE fallback
        // — not a snapshot to keep serving while connected. The old cache-first
        // logic only refetched when the cache was empty, so after a repayment the
        // dashboard, success screen and history all kept showing the pre-payment
        // balance until logout (a customer could pay twice).
  ...
  Future<Either<Failure, List<Repayment>>> getPaymentHistory() =>
      guard(() async {
        // Network-first when online (same reasoning as getLedger): a freshly-made
        // repayment must appear here, and the cache-first read hid it until the
  ...
  Future<Either<Failure, CollectionRecord>> requestCollection({
    ...
    String? idempotencyKey,
      ...
      guard(() async {
        ...
        final record = await dataSource.requestCollection(
          amount: amount,
