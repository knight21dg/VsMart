...
class BillingBackendDataSource implements BillingDataSource {
  ...
  Future<CreditSummary> getCreditSummary() async {
    ...
    // `outstanding` is the server's running ledger balance — authoritative, and
    // correct even when the ledger list endpoint paginates (it returns 20 rows,
    // so summing it client-side undercounts any account with more history).
    return (limit: _num(j['creditLimit']), outstanding: _num(j['outstanding']));
  }
  ...
  Future<CollectionRecord> requestCollection({
    ...
    final res = await _client.post<dynamic>(
      ...
      options: Options(
        headers: {
          // Prefer the caller's stable key so a retried submit (or a timed-out
          // request that actually reached the server) never creates two pickups.
          // Only fall back to a generated key when none is supplied.
          'Idempotency-Key': idempotencyKey ??
              'collect_${DateTime.now().microsecondsSinceEpoch}',
        },
      ),
    );
    final j = _obj(res.data);
