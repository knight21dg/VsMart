...
`FLAVOR=prod` turns off debug logging + the auth bypass. `MAPS_API_KEY` is the
**Dart-visible** copy of the key, used for the app's direct Places (location
search) calls — separate from the native manifest/Info.plist copy that renders
map tiles. Both are the same one key (see repo-root `MAPS_SETUP.md`).
