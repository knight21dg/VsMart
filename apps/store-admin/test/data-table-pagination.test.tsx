...
describe("DataTable pagination", () => {
  ...
  it("shows the empty message rather than an error when a page is genuinely empty", async () => {
    render(<DataTable columns={columns} data={[]} emptyMessage="No orders match your search." />);
    expect(await screen.findByText("No orders match your search.")).toBeTruthy();
  });

  it("shows skeletons, not the empty message, while loading", () => {
    render(<DataTable columns={columns} data={[]} loading emptyMessage="No orders found." />);
