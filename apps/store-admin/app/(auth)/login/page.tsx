...
export default function LoginPage() {
  ...
  React.useEffect(() => {
    ...
    if (p.get("denied")) setNotice("That account isn't a staff member of any store.");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Full navigation so the StoreProvider remounts with the new token and
      // resolves /store/me fresh (avoids a stale gate on client-side replace).
  ...
  return (
    <div className="flex min-h-screen w-full bg-white">
      {/* ── Left: branded panel ───────────────────────────────────────── */}
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-gradient-to-br from-[#7cb342] via-[#558b2f] to-[#1b3d12] p-12 text-white lg:flex">
        ...
        <div className="relative max-w-md">
          ...
          <p className="mt-6 text-base leading-relaxed text-white/80">
            Run your store front to back — billing &amp; POS, stock and purchases,
            collections, returns and daily reports. Sign in to your Store Panel.
          </p>
          <span className="mt-7 inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white/90 ring-1 ring-white/20">
            <Store className="size-3.5" /> Store staff sign-in
          </span>
        </div>
      ...
      <div className="flex w-full flex-col justify-center px-6 py-12 sm:px-12 lg:w-1/2 lg:px-20">
        <div className="mx-auto w-full max-w-sm">
          ...
          <div className="mb-10 flex items-center gap-2 lg:hidden">
            ...
            <span className="text-base font-semibold tracking-tight">VS Mart Store</span>
          </div>

          <h2 className="font-display text-2xl font-bold tracking-tight">Welcome back!</h2>
          <p className="mt-1.5 text-sm text-muted-foreground">
            Sign in to the VS Mart Store Panel.
          </p>
          ...
          <p className="mt-10 text-center text-[11px] text-muted-foreground">
            Access is limited to authorised store staff.
          </p>
        </div>
      </div>
    </div>
